...
OperatorAddCookie::exec(const Resources &res) const
{
  ...
  if (res.bufp && res.hdr_loc) {
    ...
    std::string updated_cookie;
    bool cookie_modified =
      CookieHelper::cookieModifyHelper(cookies, cookies_len, updated_cookie, CookieHelper::COOKIE_OP_ADD, _cookie, value);
    if (cookie_modified) {
      if (TS_SUCCESS ==
          TSMimeHdrFieldValueStringSet(res.bufp, res.hdr_loc, field_loc, -1, updated_cookie.c_str(), updated_cookie.size())) {
...
// OperatorUpdateCookie
void
OperatorUpdateCookie::initialize(Parser &p)
{
  OperatorCookies::initialize(p);
  _value.set_value(p.get_value());
}
...
void
OperatorUpdateCookie::exec(const Resources &res) const
{
  std::string value;
  ...
  if (res.bufp && res.hdr_loc) {
    TSDebug(PLUGIN_NAME, "OperatorUpdateCookie::exec() invoked on cookie %s", _cookie.c_str());
    TSMLoc field_loc;

    // Find Cookie
    field_loc = TSMimeHdrFieldFind(res.bufp, res.hdr_loc, TS_MIME_FIELD_COOKIE, TS_MIME_LEN_COOKIE);
    if (field_loc == NULL) {
      TSDebug(PLUGIN_NAME, "OperatorUpdateCookie::exec, no cookie");
      if (TS_SUCCESS == TSMimeHdrFieldCreateNamed(res.bufp, res.hdr_loc, TS_MIME_FIELD_COOKIE, TS_MIME_LEN_COOKIE, &field_loc)) {
        value = _cookie + "=" + value;
        if (TS_SUCCESS == TSMimeHdrFieldValueStringSet(res.bufp, res.hdr_loc, field_loc, -1, value.c_str(), value.size())) {
          TSDebug(PLUGIN_NAME, "Adding cookies %s", _cookie.c_str());
    ...
    std::string updated_cookie;
    bool cookie_modified =
      CookieHelper::cookieModifyHelper(cookies, cookies_len, updated_cookie, CookieHelper::COOKIE_OP_UPD, _cookie, value);
    if (cookie_modified) {
      if (TS_SUCCESS ==
          TSMimeHdrFieldValueStringSet(res.bufp, res.hdr_loc, field_loc, -1, updated_cookie.c_str(), updated_cookie.size())) {
        TSDebug(PLUGIN_NAME, "OperatorUpdateCookie::exec, updated_cookie = [%s]", updated_cookie.c_str());
      }
    }
    TSHandleMLocRelease(res.bufp, res.hdr_loc, field_loc);
  }
                                 ...
                                 const std::string &cookie_value)
{
  ...
  for (size_t idx = 0; idx < cookies_len;) {
    ...
    for (; idx < cookies_len && std::isspace(cookies[idx]); idx++)
      ;
    if (strncmp(cookies + idx, cookie_key.c_str(), cookie_key.size()) == 0) {
      ...
      if (idx < cookies_len && cookies[idx++] == '=') {
        ...
        if (cookie_op == CookieHelper::COOKIE_OP_ADD) {
          return false;
        }
        for (; idx < cookies_len && std::isspace(cookies[idx]); idx++)
          ;
        ...
        if (cookie_op == CookieHelper::COOKIE_OP_UPD) {
        ...
        if (cookie_op == CookieHelper::COOKIE_OP_DEL) {
          // +1 to skip the semi-colon after the cookie_value
          updated_cookies =
            std::string(cookies, key_start_idx) +
            (value_end_idx >= cookies_len ? "" : std::string(cookies + value_end_idx + 1, cookies_len - value_end_idx - 1));
  ...
  if (cookie_op == CookieHelper::COOKIE_OP_ADD || cookie_op == CookieHelper::COOKIE_OP_UPD) {
    if (cookies_len == 0) {

...
#define PLUGIN_NAME "request_buffer_transform"
...
my_data_alloc()
{
  TSDebug(PLUGIN_NAME,"my_data_alloc starting");
  MyData *data;
...
{
  TSDebug(PLUGIN_NAME,"my_data_destroy starting");
  if (data) {
...
{
  TSDebug(PLUGIN_NAME,"handle_buffering starting");
  TSVIO write_vio;
  int towrite;
  int avail;
  ...
  if (TSVIONTodoGet(write_vio) > 0) {
    ...
    TSDebug(PLUGIN_NAME,"Write vio has done with [%lld] bytes.", TSVIONDoneGet(write_vio));
    data->state = STATE_OUTPUT_DATA;
...
{
  TSDebug(PLUGIN_NAME,"handle_output starting");
...
{
  TSDebug(PLUGIN_NAME,"handle_transform starting");
  MyData *data;
  int done;
...
static int
request_buffer_transform(TSCont contp, TSEvent event, void *edata ATS_UNUSED)
{
  TSDebug(PLUGIN_NAME,"request_buffer_transform starting");
...
{
  /*TODO: instead of return 1 for all, we only interested in post and put requests. */
  return 1;
  /*TSMBuffer bufp;
...
{
  TSDebug(PLUGIN_NAME,"transform_add starting");
  TSVConn connp;

  connp = TSTransformCreate(request_buffer_transform, txnp);
  //TSHttpTxnHookAdd(txnp, TS_HTTP_REQUEST_BUFFER_HOOK, connp);
...
TSPluginInit(int argc ATS_UNUSED, const char *argv[] ATS_UNUSED)
{
  ...
  TSHttpHookAdd(TS_HTTP_READ_REQUEST_HDR_HOOK, TSContCreate(transform_plugin, mutex));
